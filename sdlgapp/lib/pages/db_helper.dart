...
class SQLHelper {
  ...
  static Future<Map<String, dynamic>?> verifyUser(
    ...
    try {
      ...
      if (result.isNotEmpty) {
        print("✅ Usuario autenticado: ${result.first['nombre']}");
        return result.first;
      } else {
  ...
  // Método para insertar usuarios de prueba
  static Future<void> insertDefaultUsers() async {
    final db = await SQLHelper.db();

    try {
      // Verificar si ya existen usuarios
      final existingUsers = await db.query('tusuarios');
      ...
      for (var user in defaultUsers) {
        ...
        print("Usuario insertado: ${user['usuario']}");
      }

      print("✅ Usuarios por defecto insertados correctamente");
    } catch (e) {
  ...
  // Obtener registros por arete Y tipo de animal (este sirva para mostrarlos independientemente de si son becerros o animales adultos)
